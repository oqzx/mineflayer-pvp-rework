...
export class PredictionLayer {
  ...
  update(target: Entity, botPosition: Vec3, tick: number, profile: EnemyProfile): PredictionFrame {
    ...
    const retreatProbability = isFleeingLikely ? 0.7 : sigmoid((profile.aggressionScore - 0.5) * -6) * 0.4;
    ...
    const postHitBehaviorPredicted: 'aggressive' | 'defensive' | 'neutral' =
      profile.aggressionScore > 0.65 ? 'aggressive' : profile.blockingRatio > 0.5 ? 'defensive' : 'neutral';
  ...
  private recordVelocitySample(target: Entity, tick: number): void {
    this.velocityHistory.push({ velocity: target.velocity.clone(), position: target.position.clone(), tick });
  ...
  private computeHitChance(target: Entity, botPos: Vec3, isTurning: boolean, velMag: number): number {
