...
export class SessionMemory {
  ...
  recordAttack(entityId: number, tick: number, blocking: boolean, fromAbove: boolean, reachDistance: number): void {
  ...
  recordBehavior(
    ...
    const profile = this.getOrCreate(entityId);
    profile.behaviorSamples.push({ tick, usedShield, usedBow, usedPearl, usedPotion, usedGapple, usedAxe });
  ...
  recordKnockbackResponse(entityId: number, resistedKb: boolean): void {
    const profile = this.getOrCreate(entityId);
    profile.knockbackResistanceEstimate = ewma(profile.knockbackResistanceEstimate, resistedKb ? 1 : 0, 0.12);
  }
  ...
  private recomputeSwingInterval(profile: EnemyProfile): void {
    ...
    profile.latencyEstimate = ewma(profile.latencyEstimate, profile.swingIntervalVariance * 20, 0.08);
  }
  ...
  private recomputeStrafeDir(profile: EnemyProfile): void {
    ...
    if (lateralSamples.length > 0) {
      const avgLateral = lateralSamples.reduce((a, s) => a + s.velocityMagnitude, 0) / lateralSamples.length;
  ...
  private recomputeVelocityStats(profile: EnemyProfile): void {
    ...
    profile.velocityMagnitudeVariance = ewma(profile.velocityMagnitudeVariance, Math.sqrt(variance), 0.12);
  }
  ...
  private recomputeHitFromAbove(profile: EnemyProfile): void {
    ...
    profile.hitFromAboveFrequency = ewma(profile.hitFromAboveFrequency, fromAbove / recent.length, 0.1);
  }
  ...
  private recomputeBurstDamage(profile: EnemyProfile): void {
    ...
    if (tickSpan > 0) profile.burstDamageRate = ewma(profile.burstDamageRate, totalDamage / tickSpan, 0.1);
  }
  ...
  private recomputeBehaviorRatios(profile: EnemyProfile): void {
    ...
    profile.bowUsageRatio = ewma(profile.bowUsageRatio, recent.filter((s) => s.usedBow).length / total, 0.1);
    profile.pearlUsageRatio = ewma(profile.pearlUsageRatio, recent.filter((s) => s.usedPearl).length / total, 0.1);
    profile.potionUsageRatio = ewma(profile.potionUsageRatio, recent.filter((s) => s.usedPotion).length / total, 0.1);
    profile.gappleUsageRatio = ewma(profile.gappleUsageRatio, recent.filter((s) => s.usedGapple).length / total, 0.1);
    profile.axeUsageRatio = ewma(profile.axeUsageRatio, recent.filter((s) => s.usedAxe).length / total, 0.1);
