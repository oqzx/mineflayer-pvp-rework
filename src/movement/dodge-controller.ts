...
export class ProjectileScanner {
    ...
    record(entity: Entity): void {
        ...
        samples.push({
            pos: entity.position.clone(), time: Date.now()
        })
    ...
    private deriveVelocity(entity: Entity): Vec3 {
        ...
        if (vectorMagnitude(reported) > MIN_VELOCITY_THRESHOLD) return reported.clone()
    ...
    private hitsTargetViaLibrary(
        ...
        totalTicks: number; impactPosition: Vec3
    } | null {
        ...
        const result = ShotFactory.fromEntity(patched, this.intercepter).hitsEntity(aabb)
        if (!result || result.shotInfo.nearestDistance > 0) return null

        return {
            totalTicks: result.shotInfo.totalTicks,
            impactPosition: (result as {
                intersectPos?: Vec3
            }).intersectPos ?? projectile.position.clone(),
        }
    }
