...
function resolveJumpBoostEffectId(bot: Bot): number {
  ...
  return (
    registry?.effectsByName?.['jump_boost']?.id ?? registry?.effectsByName?.['Jump Boost']?.id ?? 8
  )
}
...
export class HeightAdvantage {
  ...
  shouldUseJumpBoostPotion(bot: Bot, _situation: 'height' | 'escape' | 'tower'): boolean {
    ...
    const potion = bot.inventory
      .items()
      .find((i) => i.name.includes('jump_boost') || i.name.includes('leaping'))
